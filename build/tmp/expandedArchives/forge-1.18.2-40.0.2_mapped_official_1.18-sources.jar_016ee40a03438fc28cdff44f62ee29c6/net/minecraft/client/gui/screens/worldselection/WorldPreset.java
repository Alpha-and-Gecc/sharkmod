...
public abstract class WorldPreset {
   ...
   private static final WorldPreset FLAT = new WorldPreset("flat") {
      protected ChunkGenerator generator(RegistryAccess p_194099_, long p_194100_) {
         ...
         Registry<StructureSet> registry1 = p_194099_.registryOrThrow(Registry.f_211073_);
   ...
   private static final WorldPreset DEBUG = new WorldPreset("debug_all_block_states") {
      protected ChunkGenerator generator(RegistryAccess p_194111_, long p_194112_) {
         return new DebugLevelSource(p_194111_.registryOrThrow(Registry.f_211073_), p_194111_.registryOrThrow(Registry.BIOME_REGISTRY));
      }
   };
   ...
   protected static final Map<Optional<WorldPreset>, WorldPreset.PresetEditor> EDITORS = ImmutableMap.of(Optional.of(FLAT), (p_205498_, p_205499_) -> {
      ...
      RegistryAccess registryaccess = p_205498_.worldGenSettingsComponent.m_205473_();
      ...
      return new CreateFlatWorldScreen(p_205498_, (p_210935_) -> {
         p_205498_.worldGenSettingsComponent.updateSettings(new WorldGenSettings(p_205499_.seed(), p_205499_.generateFeatures(), p_205499_.generateBonusChest(), WorldGenSettings.m_204649_(registry2, p_205499_.m_204655_(), new FlatLevelSource(registry1, p_210935_))));
      }, chunkgenerator instanceof FlatLevelSource ? ((FlatLevelSource)chunkgenerator).settings() : FlatLevelGeneratorSettings.m_211734_(registry, registry1));
   }, Optional.of(SINGLE_BIOME_SURFACE), (p_205475_, p_205476_) -> {
      return new CreateBuffetWorldScreen(p_205475_, p_205475_.worldGenSettingsComponent.m_205473_(), (p_205484_) -> {
         p_205475_.worldGenSettingsComponent.updateSettings(m_205493_(p_205475_.worldGenSettingsComponent.m_205473_(), p_205476_, p_205484_));
      }, m_205490_(p_205475_.worldGenSettingsComponent.m_205473_(), p_205476_));
   });
   ...
   static NoiseBasedChunkGenerator fixedBiomeGenerator(RegistryAccess p_194086_, long p_194087_, ResourceKey<NoiseGeneratorSettings> p_194088_) {
      ...
      Registry<StructureSet> registry1 = p_194086_.registryOrThrow(Registry.f_211073_);
      ...
      return new NoiseBasedChunkGenerator(registry1, registry2, new FixedBiomeSource(registry.m_203538_(Biomes.PLAINS)), p_194087_, registry3.m_203538_(p_194088_));
   }
   ...
   private static WorldGenSettings m_205493_(RegistryAccess p_205494_, WorldGenSettings p_205495_, Holder<Biome> p_205496_) {
      ...
      Registry<StructureSet> registry1 = p_205494_.registryOrThrow(Registry.f_211073_);
      Registry<NoiseGeneratorSettings> registry2 = p_205494_.registryOrThrow(Registry.NOISE_GENERATOR_SETTINGS_REGISTRY);
      Holder<NoiseGeneratorSettings> holder = registry2.m_203538_(NoiseGeneratorSettings.OVERWORLD);
      return new WorldGenSettings(p_205495_.seed(), p_205495_.generateFeatures(), p_205495_.generateBonusChest(), WorldGenSettings.m_204649_(registry, p_205495_.m_204655_(), new NoiseBasedChunkGenerator(registry1, p_205494_.registryOrThrow(Registry.NOISE_REGISTRY), biomesource, p_205495_.seed(), holder)));
   }

   private static Holder<Biome> m_205490_(RegistryAccess p_205491_, WorldGenSettings p_205492_) {
      return p_205492_.overworld().getBiomeSource().m_207840_().stream().findFirst().orElse(p_205491_.registryOrThrow(Registry.BIOME_REGISTRY).m_203538_(Biomes.PLAINS));
   }
   ...
   public WorldGenSettings m_205485_(RegistryAccess p_205486_, long p_205487_, boolean p_205488_, boolean p_205489_) {
      return new WorldGenSettings(p_205487_, p_205488_, p_205489_, WorldGenSettings.m_204649_(p_205486_.registryOrThrow(Registry.DIMENSION_TYPE_REGISTRY), DimensionType.m_204490_(p_205486_, p_205487_), this.generator(p_205486_, p_205487_)));
   }
