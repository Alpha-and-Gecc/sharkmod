...
public class Camera {
   ...
   public FogType getFluidInCamera() {
      if (!this.initialized) {
         ...
         if (fluidstate.m_205070_(FluidTags.WATER) && this.position.y < (double)((float)this.blockPosition.getY() + fluidstate.getHeight(this.level, this.blockPosition))) {
            ...
            for(Vec3 vec3 : Arrays.asList(camera$nearplane.forward, camera$nearplane.getTopLeft(), camera$nearplane.getTopRight(), camera$nearplane.getBottomLeft(), camera$nearplane.getBottomRight())) {
               ...
               if (fluidstate1.m_205070_(FluidTags.LAVA)) {
