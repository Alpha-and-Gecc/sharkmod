...
public class BellBlockEntity extends BlockEntity {
   ...
   private void updateEntities() {
      ...
      if (!this.level.isClientSide) {
         for(LivingEntity livingentity : this.nearbyEntities) {
            if (livingentity.isAlive() && !livingentity.isRemoved() && blockpos.m_203195_(livingentity.position(), 32.0D)) {
   ...
   private static boolean areRaidersNearby(BlockPos p_155200_, List<LivingEntity> p_155201_) {
      for(LivingEntity livingentity : p_155201_) {
         if (livingentity.isAlive() && !livingentity.isRemoved() && p_155200_.m_203195_(livingentity.position(), 32.0D) && livingentity.getType().m_204039_(EntityTypeTags.RAIDERS)) {
            return true;
         }
      }
   ...
   private static void showBellParticles(Level p_155208_, BlockPos p_155209_, List<LivingEntity> p_155210_) {
      ...
      int i = (int)p_155210_.stream().filter((p_155216_) -> {
         return p_155209_.m_203195_(p_155216_.position(), 48.0D);
      }).count();
   ...
   private static boolean isRaiderWithinRange(BlockPos p_155197_, LivingEntity p_155198_) {
      return p_155198_.isAlive() && !p_155198_.isRemoved() && p_155197_.m_203195_(p_155198_.position(), 48.0D) && p_155198_.getType().m_204039_(EntityTypeTags.RAIDERS);
   }
