      ...
      if (p_40633_.getBlockState(p_40634_).is(Blocks.WATER) && p_40633_.getFluidState(p_40634_).getAmount() == 8) {
         if (!(p_40633_ instanceof ServerLevel)) {
            ...
            for(int i = 0; i < 128; ++i) {
               ...
               Holder<Biome> holder = p_40633_.m_204166_(blockpos);
               if (holder.m_203565_(Biomes.WARM_OCEAN)) {
                  if (i == 0 && p_40635_ != null && p_40635_.getAxis().isHorizontal()) {
                     blockstate = Registry.BLOCK.m_203431_(BlockTags.WALL_CORALS).flatMap((p_204098_) -> {
                        return p_204098_.m_203450_(p_40633_.random);
                     }).map((p_204100_) -> {
                        return p_204100_.m_203334_().defaultBlockState();
                     }).orElse(blockstate);
                     if (blockstate.hasProperty(BaseCoralWallFanBlock.FACING)) {
                        blockstate = blockstate.setValue(BaseCoralWallFanBlock.FACING, p_40635_);
                     }
                  } else if (random.nextInt(4) == 0) {
                     blockstate = Registry.BLOCK.m_203431_(BlockTags.UNDERWATER_BONEMEALS).flatMap((p_204091_) -> {
                        return p_204091_.m_203450_(p_40633_.random);
                     }).map((p_204095_) -> {
                        return p_204095_.m_203334_().defaultBlockState();
                     }).orElse(blockstate);
                  }
               }

               if (blockstate.m_204338_(BlockTags.WALL_CORALS, (p_204093_) -> {
