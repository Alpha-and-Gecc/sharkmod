...
public class ItemParser {
   ...
   public void readTag() throws CommandSyntaxException {
      if (!this.forTesting) {
         ...
         this.suggestions = this::m_205672_;
         this.reader.expect('#');
         this.tagCursor = this.reader.getCursor();
         this.tag = TagKey.m_203882_(Registry.ITEM_REGISTRY, ResourceLocation.read(this.reader));
      }
   }
   ...
   public ItemParser parse() throws CommandSyntaxException {
      this.suggestions = this::m_205675_;
      if (this.reader.canRead() && this.reader.peek() == '#') {
         this.readTag();
      } else {
         this.readItem();
         this.suggestions = this::m_205668_;
      }
   ...
   private CompletableFuture<Suggestions> m_205668_(SuggestionsBuilder p_205669_, Registry<Item> p_205670_) {
   ...
   private CompletableFuture<Suggestions> m_205672_(SuggestionsBuilder p_205673_, Registry<Item> p_205674_) {
      return SharedSuggestionProvider.suggestResource(p_205674_.m_203613_().map(TagKey::f_203868_), p_205673_.createOffset(this.tagCursor));
   }

   private CompletableFuture<Suggestions> m_205675_(SuggestionsBuilder p_205676_, Registry<Item> p_205677_) {
      if (this.forTesting) {
         SharedSuggestionProvider.m_205106_(p_205677_.m_203613_().map(TagKey::f_203868_), p_205676_, String.valueOf('#'));
      }

      return SharedSuggestionProvider.suggestResource(Registry.ITEM.keySet(), p_205676_);
   }

   public CompletableFuture<Suggestions> m_205665_(SuggestionsBuilder p_205666_, Registry<Item> p_205667_) {
