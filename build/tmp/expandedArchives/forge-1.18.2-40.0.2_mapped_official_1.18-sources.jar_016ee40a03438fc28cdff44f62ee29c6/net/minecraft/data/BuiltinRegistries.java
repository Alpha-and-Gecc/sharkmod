...
public class BuiltinRegistries {
   ...
   public static final Registry<ConfiguredFeature<?, ?>> CONFIGURED_FEATURE = registerSimple(Registry.CONFIGURED_FEATURE_REGISTRY, FeatureUtils::m_206469_);
   public static final Registry<PlacedFeature> PLACED_FEATURE = registerSimple(Registry.PLACED_FEATURE_REGISTRY, PlacementUtils::m_206492_);
   public static final Registry<ConfiguredStructureFeature<?, ?>> CONFIGURED_STRUCTURE_FEATURE = registerSimple(Registry.CONFIGURED_STRUCTURE_FEATURE_REGISTRY, StructureFeatures::m_206440_);
   public static final Registry<StructureSet> f_211084_ = registerSimple(Registry.f_211073_, StructureSets::m_211127_);
   ...
   public static final Registry<StructureTemplatePool> TEMPLATE_POOL = registerSimple(Registry.TEMPLATE_POOL_REGISTRY, Pools::m_206434_);
   @Deprecated public static final Registry<Biome> BIOME = forge(Registry.BIOME_REGISTRY, Biomes::m_206462_);
   public static final Registry<NormalNoise.NoiseParameters> NOISE = registerSimple(Registry.NOISE_REGISTRY, NoiseData::m_206433_);
   public static final Registry<DensityFunction> f_211085_ = registerSimple(Registry.f_211074_, NoiseRouterData::m_209468_);
   public static final Registry<NoiseGeneratorSettings> NOISE_GENERATOR_SETTINGS = registerSimple(Registry.NOISE_GENERATOR_SETTINGS_REGISTRY, NoiseGeneratorSettings::m_204599_);
   ...
   private static <T, R extends WritableRegistry<T>> R internalRegister(ResourceKey<? extends Registry<T>> p_123889_, R p_123890_, Supplier<? extends Holder<? extends T>> p_123891_, Lifecycle p_123892_) {
      ...
      WRITABLE_REGISTRY.m_203505_((ResourceKey)p_123889_, p_123890_, p_123892_);
      return p_123890_;
   }
   ...
   public static <T> Holder<T> m_206396_(Registry<T> p_206397_, String p_206398_, T p_206399_) {
      return m_206388_(p_206397_, new ResourceLocation(p_206398_), p_206399_);
   }

   public static <T> Holder<T> m_206388_(Registry<T> p_206389_, ResourceLocation p_206390_, T p_206391_) {
      return m_206384_(p_206389_, ResourceKey.create(p_206389_.key(), p_206390_), p_206391_);
   }
   ...
   static {
      LOADERS.forEach((p_206393_, p_206394_) -> {
         if (!p_206394_.get().m_203633_()) {
            LOGGER.error("Unable to bootstrap registry '{}'", (Object)p_206393_);
         }

      });
      Registry.m_205992_(WRITABLE_REGISTRY);
      f_206379_ = RegistryAccess.m_206165_(REGISTRY);
   }
}
