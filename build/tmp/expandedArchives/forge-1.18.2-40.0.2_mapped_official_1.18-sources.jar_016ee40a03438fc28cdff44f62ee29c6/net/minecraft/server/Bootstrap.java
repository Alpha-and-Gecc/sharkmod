   ...
   private static void validateThatAllBiomeFeaturesHaveBiomeFilter() {
      BuiltinRegistries.BIOME.stream().forEach((p_197754_) -> {
         ...
         list.stream().flatMap(HolderSet::m_203614_).forEach((p_206844_) -> {
            if (!((PlacedFeature)p_206844_.m_203334_()).placement().contains(BiomeFilter.biome())) {
               Util.logAndPauseIfInIde("Placed feature " + BuiltinRegistries.PLACED_FEATURE.getResourceKey((PlacedFeature)p_206844_.m_203334_()) + " is missing BiomeFilter.biome()");
            }
