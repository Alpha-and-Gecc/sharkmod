...
public interface CubicSpline<C> extends ToFloatFunction<C> {
   ...
   CubicSpline<C> m_211396_(CubicSpline.CoordinateVisitor<C> p_211579_);
   ...
   public static record Constant<C>(float value) implements CubicSpline<C> {
      ...
      public CubicSpline<C> m_211396_(CubicSpline.CoordinateVisitor<C> p_211581_) {
         return this;
      }
   }

   public interface CoordinateVisitor<C> {
      ToFloatFunction<C> m_211582_(ToFloatFunction<C> p_211583_);
   }
   ...
   public static record Multipoint<C>(ToFloatFunction<C> coordinate, float[] locations, List<CubicSpline<C>> values, float[] derivatives) implements CubicSpline<C> {
      ...
      public float m_207241_() {
         return (float)this.values().stream().mapToDouble(CubicSpline::m_207241_).min().orElseThrow();
      }

      public float m_207240_() {
         return (float)this.values().stream().mapToDouble(CubicSpline::m_207240_).max().orElseThrow();
      }

      public CubicSpline<C> m_211396_(CubicSpline.CoordinateVisitor<C> p_211585_) {
         return new CubicSpline.Multipoint<>(p_211585_.m_211582_(this.coordinate), this.locations, this.values().stream().map((p_211588_) -> {
            return p_211588_.m_211396_(p_211585_);
