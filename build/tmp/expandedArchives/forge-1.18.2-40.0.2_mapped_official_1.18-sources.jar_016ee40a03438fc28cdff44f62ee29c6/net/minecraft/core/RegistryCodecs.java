...
public class RegistryCodecs {
   private static <T> MapCodec<RegistryCodecs.RegistryEntry<T>> m_206303_(ResourceKey<? extends Registry<T>> p_206304_, MapCodec<T> p_206305_) {
      return RecordCodecBuilder.mapCodec((p_206309_) -> {
         return p_206309_.group(ResourceLocation.CODEC.xmap(ResourceKey.elementKey(p_206304_), ResourceKey::location).fieldOf("name").forGetter(RegistryCodecs.RegistryEntry::f_206354_), Codec.INT.fieldOf("id").forGetter(RegistryCodecs.RegistryEntry::f_206355_), p_206305_.forGetter(RegistryCodecs.RegistryEntry::f_206356_)).apply(p_206309_, RegistryCodecs.RegistryEntry::new);
      });
   }
   ...
   public static <T> Codec<Registry<T>> m_206291_(ResourceKey<? extends Registry<T>> p_206292_, Lifecycle p_206293_, Codec<T> p_206294_) {
      return m_206303_(p_206292_, p_206294_.fieldOf("element")).codec().listOf().xmap((p_206298_) -> {
   ...
   public static <E> Codec<Registry<E>> m_206318_(ResourceKey<? extends Registry<E>> p_206319_, Lifecycle p_206320_, Codec<E> p_206321_) {
      Codec<Map<ResourceKey<E>, E>> codec = m_206315_(p_206319_, p_206321_);
      Encoder<Registry<E>> encoder = codec.comap((p_206271_) -> {
         return ImmutableMap.copyOf(p_206271_.entrySet());
      });
      return Codec.of(encoder, m_206282_(p_206319_, p_206321_, codec, p_206320_), "DataPackRegistryCodec for " + p_206319_);
   }
   ...
   private static <E> Decoder<Registry<E>> m_206282_(final ResourceKey<? extends Registry<E>> p_206283_, final Codec<E> p_206284_, Decoder<Map<ResourceKey<E>, E>> p_206285_, Lifecycle p_206286_) {
      final Decoder<WritableRegistry<E>> decoder = p_206285_.map((p_206302_) -> {
         ...
         p_206302_.forEach((p_206275_, p_206276_) -> {
            writableregistry.m_203505_(p_206275_, p_206276_, p_206286_);
         });
         return writableregistry;
      });
      return new Decoder<Registry<E>>() {
         public <T> DataResult<Pair<Registry<E>, T>> decode(DynamicOps<T> p_206352_, T p_206353_) {
            ...
            if (p_206352_ instanceof RegistryOps) {
               ...
               return registryops.m_206812_().map((p_206338_) -> {
                  return this.m_206339_(dataresult, registryops, p_206338_.f_206784_());
               }).orElseGet(() -> {
         ...
         private <T> DataResult<Pair<Registry<E>, T>> m_206339_(DataResult<Pair<WritableRegistry<E>, T>> p_206340_, RegistryOps<?> p_206341_, RegistryLoader p_206342_) {
            return p_206340_.flatMap((p_206350_) -> {
               return p_206342_.m_206762_(p_206350_.getFirst(), p_206283_, p_206284_, p_206341_.m_206831_()).map((p_206334_) -> {
