...
public class RegistryLoader {
   ...
   private final Map<ResourceKey<? extends Registry<?>>, RegistryLoader.ReadCache<?>> f_206751_ = new IdentityHashMap<>();
   ...
   public <E> DataResult<? extends Registry<E>> m_206762_(WritableRegistry<E> p_206763_, ResourceKey<? extends Registry<E>> p_206764_, Codec<E> p_206765_, DynamicOps<JsonElement> p_206766_) {
      Collection<ResourceKey<E>> collection = this.f_206750_.listResources(p_206764_);
      ...
      for(ResourceKey<E> resourcekey : collection) {
         dataresult = dataresult.flatMap((p_206780_) -> {
            return this.m_206767_(p_206780_, p_206764_, p_206765_, resourcekey, p_206766_).map((p_206761_) -> {
               return p_206780_;
            });
         });
      }
   ...
   <E> DataResult<Holder<E>> m_206767_(WritableRegistry<E> p_206768_, ResourceKey<? extends Registry<E>> p_206769_, Codec<E> p_206770_, ResourceKey<E> p_206771_, DynamicOps<JsonElement> p_206772_) {
      RegistryLoader.ReadCache<E> readcache = this.m_206773_(p_206769_);
      DataResult<Holder<E>> dataresult = readcache.f_206803_.get(p_206771_);
      if (dataresult != null) {
         return dataresult;
      } else {
         Holder<E> holder = p_206768_.m_203538_(p_206771_);
         readcache.f_206803_.put(p_206771_, DataResult.success(holder));
         Optional<DataResult<RegistryResourceAccess.ParsedEntry<E>>> optional = this.f_206750_.parseElement(p_206772_, p_206769_, p_206771_, p_206770_);
         ...
         if (optional.isEmpty()) {
            ...
            if (optional1.isPresent()) {
               ...
               p_206768_.m_203384_(parsedentry.fixedId(), p_206771_, parsedentry.value(), dataresult2.lifecycle());
            }
         ...
         readcache.f_206803_.put(p_206771_, dataresult1);
         return dataresult1;
      }
   }
   ...
   public RegistryLoader.Bound m_206757_(RegistryAccess.Writable p_206758_) {
      return new RegistryLoader.Bound(p_206758_, this);
   }

   public static record Bound(RegistryAccess.Writable f_206783_, RegistryLoader f_206784_) {
      public <E> DataResult<? extends Registry<E>> m_206789_(ResourceKey<? extends Registry<E>> p_206790_, Codec<E> p_206791_, DynamicOps<JsonElement> p_206792_) {
         WritableRegistry<E> writableregistry = this.f_206783_.m_206253_(p_206790_);
         return this.f_206784_.m_206762_(writableregistry, p_206790_, p_206791_, p_206792_);
      }
      ...
      public <E> DataResult<Holder<E>> m_206793_(ResourceKey<? extends Registry<E>> p_206794_, Codec<E> p_206795_, ResourceKey<E> p_206796_, DynamicOps<JsonElement> p_206797_) {
         WritableRegistry<E> writableregistry = this.f_206783_.m_206253_(p_206794_);
         return this.f_206784_.m_206767_(writableregistry, p_206794_, p_206795_, p_206796_, p_206797_);
      }
   }
